...
namespace Beetle
{
    class OnebyNAlignment : BeetleSearch
    {
        public OnebyNAlignment(Parameters prmts, BeetleControl bc, PiezoControl pc) : base(prmts, bc, pc)
        {}

        //parameters.errorflag will be used as program stop flag as well. Instance like meet criteria, unexpected high loss, failed
        ...
        private bool XYSearch()
        {
            if (searchMode == "scan")
            {
                ...
                if (!AxisScanSearch(axis: 1))
                {
                    ...
                    if (parameters.lossCurrentMax < -40.0 && !XYSquareSearch())
                    {
